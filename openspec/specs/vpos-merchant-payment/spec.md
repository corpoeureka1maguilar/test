# VPOS Merchant Payment Specification

## Purpose
Allow kiosk payment methods flagged `with_merchant` in Odoo to delegate the actual card transaction to an external VPOS terminal, keeping the terminal's checkout surface isolated in an iframe and using a documented HTTP/postMessage contract so a mock server can stand in until the real terminal is wired.
