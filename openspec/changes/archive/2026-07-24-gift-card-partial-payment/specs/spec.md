...
## Capability: payment-flow (MODIFIED)

### Scenario 8: Gift-card selection no longer hard-blocks on insufficient balance
- **Given** the current implementation disables confirmation and shows an error whenever `balance < total` (see `openspec/changes/gift-cards/specs/spec.md` Scenario 4)
- **When** this change is applied
- **Then** that hard block is removed for the case `0 < balance < total`; the flow instead proceeds per Scenario 2 above.
- **And** the case `balance >= total` (Scenario 1) and the case `balance === 0` / invalid card (Scenario 3) remain unchanged.
...
- [x] Scenario 8 documents the modified `payment-flow` behavior delta.
